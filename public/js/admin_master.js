			...
			$.ajax({
				...
				success: function(data) {
					if(data != null) {
						...
						admin.showProductsInCategory(urlArray[urlArray.length - 1]);
					}
				}
			});
					...
					if (prods)
					{
						...
						for (var i = 0; i < prods.length; i++)
						{
							...
							if (prods[i].public == true)
								...
								$('table tbody', '.products').append('<tr><td>' + i + '</td><td>' + type + '</td><td>' + articul + '</td><td>' + model + '</td><td>' + brand + '</td><td>' + price + '</td><td>' + pub + '</td><td>Действия</td></tr>');
						}
					} else
					{
